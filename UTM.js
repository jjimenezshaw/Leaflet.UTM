/*
 * Extends LatLng to convert easily to UTM WGS84 coordinates
...
export class Utm {
    ...
    constructor(x, y, zone, band, southHemi) {
        this.x = +x;
        this.y = +y;
        this.zone = zone;
        this.band = band;
    ...
    static setDefaultOptions(o) {
        // o can be an object or a function
        Utm._defaultOptions = o;
    }
    ...
    toString(options) {
        ...
        return Util.template(options.format, o);
    }

    // returns a LatLng object
    latLng(ignoreError) {
        try {
            const ll = UC().UTM2LatLon(this);
            return new LatLng(ll);
        } catch (e) {
    ...
    // convert to LatLng to check equality
    equals(other) {
        try {
...
export const utm = function(x, y, zone, band, southHemi) {
    ...
    if (typeof x === 'object' && 'x' in x && 'y' in x && 'zone' in x) {
        return new Utm(x.x, x.y, x.zone, x.band, x.southHemi);
    }
    return new Utm(x, y, zone, band, southHemi);
};

////////////////////////////
// Prototype in LatLng to get an Utm object.
// if zone is null, it is calculated.
LatLng.prototype.utm = function(zone, southHemi) {
    const dic = UC().LatLon2UTM(
        this.lat,
        this.lng,
        zone,
        southHemi);
...
// include class and method in L to replicate previous behaviour
L.Utm = Utm;
L.utm = utm;
...
function UC() {
    ...
    const _sm_EccSquared = 6.69437999013e-03;
    ...
    */
    ...
    const nBandIdx = bands.indexOf('N');

    function calcBand(lat) {
        if (lat < -80.0 || lat > 84.0) return '';
        const bandIdx = Math.floor((lat + 80.0) / 8);
